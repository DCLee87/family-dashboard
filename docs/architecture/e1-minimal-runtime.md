...
- 동시 읽기와 단일 쓰기 부하 시험(쓰기 API가 추가되는 반복에서 수행)
- 컨테이너 강제 종료 후 무결성 확인
...
```text
이미지 크기: 10,403,137 bytes
시작 시간: 약 2초
유휴 메모리: 3.789~8.578 MiB
정상 요청 메모리: 평균 10.492 MiB
최대 메모리: 11.060 MiB
평균·최대 CPU: 단기 부하 0.7323% / 1.60%
상태 API 응답시간: 평균 11.455 ms, 최대 62.398 ms
재시작 복구시간: 약 2초
SQLite 무결성: 정상·비정상 종료와 NAS 재부팅 후 통과
24시간 메모리 변화: 5.781→7.258 MiB, 최대 8.578 MiB
```
