...
package_dir="${1:-/volume1/homes/maius0513/family-dashboard-c3}"
timestamp="$(date -u +%Y%m%dT%H%M%SZ)"
backup_dir="${deployment_dir}/backups/pre-c3-${timestamp}"

test -x "$docker_bin"
test -d "$deployment_dir"
test -f "${package_dir}/family-dashboard-c3-amd64.tar"
...
"$docker_bin" load -i "${package_dir}/family-dashboard-c3-amd64.tar"
...
if ! "$docker_bin" compose run --rm family-dashboard --check-db; then
  echo "C3 database check failed. The service remains stopped." >&2
  echo "Restore with: sudo sh c3-rollback.sh ${backup_dir}" >&2
  exit 1
fi

"$docker_bin" compose up -d
echo "Backup created at: ${backup_dir}"
echo "C3 deployment started. Verify health, schedules, tasks, trash and Push."
