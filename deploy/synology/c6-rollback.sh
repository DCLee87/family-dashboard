...
if [ "$#" -ne 1 ]; then echo "Usage: sudo sh c6-rollback.sh /volume1/docker/family-dashboard/backups/pre-c6-<UTC>" >&2; exit 1; fi
docker_bin="/usr/local/bin/docker"; deployment_dir="/volume1/docker/family-dashboard"; backup_dir="$1"
case "$backup_dir" in "${deployment_dir}/backups/pre-c6-"*) ;; *) echo "Refusing backup path outside the C6 backup directory." >&2; exit 1 ;; esac
