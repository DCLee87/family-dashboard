...
if [ "$#" -ne 1 ]; then echo "Usage: sudo sh c7-rollback.sh /volume1/docker/family-dashboard/backups/pre-c7-<UTC>" >&2; exit 1; fi
docker_bin="/usr/local/bin/docker"; deployment_dir="/volume1/docker/family-dashboard"; backup_dir="$1"
case "$backup_dir" in "${deployment_dir}/backups/pre-c7-"*) ;; *) echo "Refusing backup path outside the C7 backup directory." >&2; exit 1 ;; esac
