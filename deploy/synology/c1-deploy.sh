...
package_dir="${1:-/volume1/homes/maius0513/family-dashboard-c1}"
timestamp="$(date -u +%Y%m%dT%H%M%SZ)"
backup_dir="${deployment_dir}/backups/pre-c1-${timestamp}"

test -x "$docker_bin"
test -d "$deployment_dir"
test -f "${package_dir}/family-dashboard-c1-amd64.tar"
...
"$docker_bin" load -i "${package_dir}/family-dashboard-c1-amd64.tar"
...
if ! "$docker_bin" compose run --rm family-dashboard --check-db; then
  echo "C1 database check failed. The service remains stopped." >&2
  echo "Restore with: sudo sh c1-rollback.sh ${backup_dir}" >&2
  exit 1
fi
...
echo "C1 deployment started. Verify health, schedules, existing devices and Push."
